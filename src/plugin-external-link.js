...
// View link AST.
export const externalLinkView = $view(
  linkSchema.mark,
  (ctx) => {
    return (mark, view, inline) => {
      ...
      link.addEventListener('click', function(e) {
        ...
        const curMark = node.marks.find(({ type }) => type === linkSchema.type(ctx));
  ...
  (ctx) =>
    () =>
      (state, dispatch) => {
        ...
        let uri = node.textContent.slice($from.parentOffset, $to.parentOffset);
        const href = toUrl(uri);
