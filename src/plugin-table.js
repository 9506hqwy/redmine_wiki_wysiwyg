...
// View table cell AST.
export const tableCellView = $view(
  tableCellSchema.node,
  (ctx) => {
    return (node, view, getPos, decorations, innerDecorations) => {
...
function createTableHeaderMenu(column, getPos) {
    ...
    left.addEventListener('click', function() {
      wysiwygEditor.action(callCommand(selectColCommand.key, { index: column, pos: getPos() }));
...
function createTableRowMenu(row, column, getPos) {
  ...
  addColBefore.addEventListener('click', function(e) {
    e.stopPropagation();
    wysiwygEditor.action(callCommand(selectColCommand.key, { index: column, pos: getPos() }));
  ...
  addColAfter.addEventListener('click', function(e) {
    e.stopPropagation();
    wysiwygEditor.action(callCommand(selectColCommand.key, { index: column, pos: getPos() }));
  ...
  addRowBefore.addEventListener('click', function(e) {
    e.stopPropagation();
    wysiwygEditor.action(callCommand(selectRowCommand.key, { index: row, pos: getPos() }));
  ...
  addRowAfter.addEventListener('click', function(e) {
    e.stopPropagation();
    wysiwygEditor.action(callCommand(selectRowCommand.key, { index: row, pos: getPos() }));
  ...
  const menu = document.createElement('div');
  menu.classList.add('menu');
  menu.append(addColBefore, delCol, addColAfter, addRowBefore, delRow, addRowAfter);
