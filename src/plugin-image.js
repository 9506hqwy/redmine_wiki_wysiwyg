...
function findExistingAttachment(name) {
  const attachment = document.querySelector(`span.existing-attachment:has(input.filename[value="${name}"])`);
  if (attachment) {
...
function findNewAttachment(name) {
  for (const attachment of document.querySelectorAll('span.attachments_form span.attachments_fields span')) {
